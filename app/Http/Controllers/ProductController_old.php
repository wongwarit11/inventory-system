...

    {
        if ($response = $this->authorizeStaffAccess()) {
            return $response;
                ...
                // ดึงค่าคำค้นหาจาก URL ถ้าไม่มีให้เป็นค่าว่าง
        ...
        // ถ้ามีค่าค้นหา ให้เพิ่มเงื่อนไขการค้นหาแบบหลายคอลัมน์
        if ($search) {
        ...
        // นำ query ที่ได้ไปทำการแบ่งหน้า
        $products = $query->paginate(15);

        // ส่งตัวแปร $products ไปที่หน้า View
        return view('products.index', compact('products'));

        // โหลดความสัมพันธ์ของหมวดหมู่และผู้จำหน่าย
        // $products = Product::with(['category', 'supplier', 'manufacturer', 'productType'])->orderBy('name')->paginate(15);
        // return view('products.index', compact('products'));
    }
