...
@section('title', 'เพิ่มแผนกใหม่') {{-- กำหนด Title เฉพาะสำหรับหน้านี้ --}}
...
@section('content') {{-- เนื้อหาของหน้านี้จะถูกแทรกเข้าไปใน @yield('content') ของ layouts/app.blade.php --}}
    <div class="container py-4"> {{-- ใช้ .container เพื่อให้ได้ max-width ที่กำหนดใน app.blade.php --}}
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1><i class="fas fa-building me-2 text-primary"></i> เพิ่มแผนกใหม่</h1>
                ...
                <form id="departmentForm" action="{{ route('departments.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="name" class="form-label fw-bold">
                            ...
                            <i class="fas fa-info-circle ms-1 custom-tooltip-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="ป้อนชื่อแผนก เช่น 'ฝ่ายขาย', 'ฝ่ายบุคคล', 'ฝ่ายการตลาด'"></i>
                        </label>
                        <input type="text" class="form-control form-control-lg rounded-pill @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" required placeholder="กรอกชื่อแผนก">
                        @error('name')
                    ...
                    <div class="mb-3">
                        <label for="description" class="form-label fw-bold">
                            คำอธิบาย
                            <i class="fas fa-info-circle ms-1 custom-tooltip-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="คำอธิบายเพิ่มเติมเกี่ยวกับหน้าที่ความรับผิดชอบของแผนก (ไม่บังคับ)"></i>
                        </label>
                        <textarea class="form-control rounded-3 @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="คำอธิบายเพิ่มเติมเกี่ยวกับแผนก">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <select class="form-select form-select-lg rounded-pill @error('status') is-invalid @enderror" id="status" name="status" required>
                            ...
                            <option value="inactive" {{ old('status', 'inactive') == 'inactive' ? 'selected' : '' }}>ไม่ใช้งาน</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
