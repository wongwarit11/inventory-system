...
@section('title', 'จ่ายออกสินค้า')

@section('content')
    <div class="container py-4"> {{-- เพิ่ม .container เพื่อให้ได้ max-width ที่กำหนดใน app.blade.php --}}
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1><i class="fas fa-arrow-alt-circle-up me-2 text-primary"></i> จ่ายออกสินค้า</h1>
        ...
        <div class="card shadow-lg rounded-4">
            <div class="card-body p-4">
                <form action="{{ route('stock_transactions.issue.store') }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="batch_id" class="form-label fw-bold">
                                    ...
                                    <i class="fas fa-info-circle custom-tooltip-icon ms-1"
                                        ...
                                        title="เลือกล็อตสินค้าที่ต้องการเบิกจ่าย,(จำเป็น)"></i>
                                </label>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="quantity" class="form-label fw-bold">
                                    จำนวนที่จ่ายออก <span class="text-danger">*</span>
                                    <i class="fas fa-info-circle custom-tooltip-icon ms-1"
                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="ระบุจำนวนสินค้าที่ต้องการเบิกจ่าย,(จำเป็น)"></i>
                                </label>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="transaction_date" class="form-label fw-bold">
                                    ...
                                    <i class="fas fa-info-circle custom-tooltip-icon ms-1"
                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="ระบุวันที่ทำรายการ,(จำเป็น)"></i>
                                </label>
                    ...
                    <div class="mb-3">
                        <label for="department_id" class="form-label fw-bold">
                            แผนกที่เบิก <span class="text-danger">*</span>
                            <i class="fas fa-info-circle custom-tooltip-icon ms-1"
                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="ระบุแผนกที่ยื่นใบขอเบิกสินค้าเช่น:'แผนกอุบัติเหตุ-ฉุกเฉิน',(จำเป็น)"></i>
                        </label>
                    ...
                    <div class="mb-4">
                        <label for="notes" class="form-label fw-bold">
                            ...
                            <i class="fas fa-info-circle custom-tooltip-icon ms-1"
                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="ระบุหมายเหตุเพิ่มเติมเกี่ยวกับการเบิกจ่ายสินค้า"></i>
                        </label>
                        <textarea class="form-control rounded-3 @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3" placeholder="หมายเหตุเพิ่มเติมเกี่ยวกับการจ่ายออกสินค้า">{{ old('notes') }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <button type="submit" id="submitButton" class="btn btn-primary btn-lg shadow-sm rounded-pill px-5 py-2 w-100">
                        <span id="buttonText"><i class="fas fa-save me-2"></i>บันทึกการจ่ายออก</span>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateBatchQuantityDisplay() {
                ...
                if (selectedBatch) {
                    ...
                    document.getElementById('quantity').max = selectedBatch.quantity; // Set max for quantity input
                } else {
                    batchQuantityDisplay.textContent = '';
                    document.getElementById('quantity').removeAttribute('max'); // Remove max if no batch selected
                }
            }
            ...
            function handleBarcode(decodedText) {
                ...
                .then(result => {
                    if (result.status === 200) {
                        ...
                        setScannerStatus(result.body.message || 'ไม่พบบาร์โค้ดนี้', true);
                    }
                })
                .catch(() => {
